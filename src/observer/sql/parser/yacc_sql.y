...
%union {
  ...
  std::vector<AggregationSqlNode> * aggregation_list;
...
insert_stmt:        /*insert   语句的语法解析树*/
    INSERT INTO ID VALUES LBRACE value value_list RBRACE value_list_list
    {
      $$ = new ParsedSqlNode(SCF_INSERT);
      $$->insertion.relation_name = $3;
      if ($9 != nullptr) {
        $$->insertion.values_list.swap(*$9);
      }
      $7->emplace_back(*$6);
      std::reverse($7->begin(), $7->end());
      $$->insertion.values_list.emplace_back(*$7);
...
value_list_list: 
    /* empty */
    {
	  ...
	  | COMMA LBRACE value value_list RBRACE value_list_list { 
      if ($6 != nullptr) {
        $$ = $6;
      } else {
...
aggregation_list:
    ...
    | COMMA aggregation aggregation_list {
      if ($3 != nullptr) {
        $$ = $3;
      } else {
        $$ = new std::vector<AggregationSqlNode>;
